...
<title>My Cart</title>
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/cssFiles/cart.css">
</head>
			...
			<tbody>
				...
				<tr>
					<td><img
						...
						height="100px" width="100px" alt="loading"></td>
					<td><%=item.getName()%></td>
					...
					<td>
						<form action="<%=request.getContextPath()%>/controller"
							...
							<input type="hidden" name="command" value="viewCart"> <input
								type="hidden" name="action" value="dec"> <input
						...
						<form action="<%=request.getContextPath()%>/controller"
							...
							<input type="number" name="enterQuantity"
								value="<%=item.getQuantity()%>" min="1" max="100" step="1"
						...
						<form action="<%=request.getContextPath()%>/controller"
							method="post" style="display: inline;">
							<input type="hidden" name="command" value="viewCart"> <input
								type="hidden" name="action" value="inc"> <input
								type="hidden" name="productId" value="<%=item.getId()%>">
							<button type="submit">+</button>
						</form>
					</td>
		...
		<div class="buttons">
			...
			<form action="<%=request.getContextPath()%>/controller" method="post"
				...
				<input type="hidden" name="command" value="listProducts"> <input
					...
					value="<%=session.getAttribute("category")%>">
